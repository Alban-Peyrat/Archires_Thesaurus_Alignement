# -*- coding: utf-8 -*- 

# Etxernal import
from enum import Enum

class Step(Enum):
    EXACT = 100
    NORMALIZED = 200
    NORMALIZED_STOP_WORDS = 230
    NORMALIZED_STOP_WORDS_EXTENDED = 261
    NORMALIZED_NO_PARENTHESIS = 300
    NORMALIZED_NO_PARENTHESIS_STOP_WORDS = 330
    NORMALIZED_NO_PARENTHESIS_STOP_WORDS_EXTENDED = 361
    FINGERPRINT = 400
    FINGERPRINT_STOP_WORDS = 430
    FINGERPRINT_STOP_WORDS_EXTENDED = 461

class TSV_Headers(Enum):
    EXT_ID = "external_ID"
    EXT_NAME = "external_label"
    EXT_IS_PREF = "is_prefLabel"
    INT_ID = "matched_internal_IDs"
    INT_NAMES = "matched_internal_label"
    NB_MATCH = "nb_match"
    STEP = "STEP"
    KEY_USED = "KEY_USED"

class TSV_Synthesis_Headers(Enum):
    EXT_ID = "external_ID"
    EXT_PREFLABEL = "external_prefLabel"
    EXT_ALTLABEL = "external_altLabel"
    INT_ID = "matched_internal_IDs"
    INT_PREFLABEL = "matched_internal_prefLabels"
    INT_ALTLABEL = "matched_internal_altLabels"
    NB_MATCH = "nb_match"
    STEP = "Most precise Step"